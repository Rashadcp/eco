...
void main() {
  ...
  gl_FragColor = color;
      ...
      const updateSize = () => {
        ...
        (uniforms.iResolution.value as [number, number]) = [w * renderer.dpr, h * renderer.dpr];
      };

      const loop = (t: number) => {
        if (!rendererRef.current || !uniformsRef.current || !meshRef.current) return;
        (uniformsRef.current.iTime.value as number) = t * 0.001;
        try {
  ...
  useEffect(() => {
    ...
    (u.iRayColor1.value as [number, number, number]) = hexToRgb(rayColor1);
    (u.iRayColor2.value as [number, number, number]) = hexToRgb(rayColor2);
